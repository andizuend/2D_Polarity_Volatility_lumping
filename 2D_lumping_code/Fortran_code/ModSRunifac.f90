...
!==========================================================================================================================
    ...
    if (calcviscosity .or. grefcallID > 0) then
        ...
        !special treatment for systems containing PEG oligomers; not an ideal approach but a feasible one for now;
            ...
            if (isPEGsystem) then 
                do i = 1,nneutral
                    if (ITAB(i,154) > 1) then       !molecules with more than one oxyethylene group are flagged for special treatment
                        XieC(i) = min(exp(lnGaC(i))*X(i), 1.0E0_wp)*ln_eta0(i)  !for PEG components, limit combinatorial activity to between 0 and 1 for viscosity contribution
                    endif
                enddo
            endif
        else
